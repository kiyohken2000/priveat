...
const buildAdviceMessages = (context, kind) => {
  let ask
  if (kind === 'past') {
    ask = 'この日の記録を踏まえて、振り返りと次に活かせるポイントを短くアドバイスしてください。'
  } else if (kind === 'weekly') {
    ask = '直近 7 日間の傾向を踏まえて、来週に向けた改善ヒントや継続したい良い習慣を短くアドバイスしてください。'
  } else {
    ask = '今日の記録を踏まえて、残り時間の過ごし方や明日への小さなヒントを短くアドバイスしてください。'
  }
  return [
    {
      role: 'system',
...
// 週始め頃 (weekStart ±3 日) と週終わり頃 (weekEnd ±3 日) の体重差。
const getWeightChangeInWeek = async (db, weekStart, weekEnd) => {
  const startRow = await db.getFirstAsync(
    `SELECT weight_kg, measured_at FROM weight_log
      WHERE date(measured_at,'localtime') BETWEEN
            date(?, '-3 days') AND date(?, '+3 days')
      ORDER BY ABS(julianday(date(measured_at,'localtime')) - julianday(?)) ASC
      LIMIT 1`,
    [weekStart, weekStart, weekStart],
  )
  const endRow = await db.getFirstAsync(
    `SELECT weight_kg, measured_at FROM weight_log
      WHERE date(measured_at,'localtime') BETWEEN
            date(?, '-3 days') AND date(?, '+3 days')
      ORDER BY ABS(julianday(date(measured_at,'localtime')) - julianday(?)) ASC
  ...
  return {
    ...
    delta: startRow && endRow ? endRow.weight_kg - startRow.weight_kg : null,
  }
}

export const buildAdviceContextForWeek = async (weekStart) => {
  const db = getDb()
  const weekEnd = addDays(weekStart, 6)
  const [profile, latestWeight, week, weight, stance] = await Promise.all([
    getProfile(),
    getLatestWeight(),
    getWeekValues(db, weekStart, weekEnd),
    getWeightChangeInWeek(db, weekStart, weekEnd),
    getStance(),
  ])
  ...
  lines.push(`【直近7日 (${formatDateLabel(weekStart)} 〜 ${formatDateLabel(weekEnd)})】`)
  if (week.intakeDays > 0) {
    lines.push(`- 摂取: 平均 ${round(week.intakeAvg)}kcal/日 (記録 ${week.intakeDays}/7 日)`)
  } else {
    lines.push('- 摂取: 記録なし')
  }
  if (week.activeAvg != null) {
    const stepStr = week.stepsAvg != null ? ` / 平均 ${Math.round(week.stepsAvg).toLocaleString()}歩/日` : ''
    lines.push(`- 運動消費: 平均 ${round(week.activeAvg)}kcal/日${stepStr} (記録 ${week.activeDays}/7 日)`)
  }
  if (bmr != null) lines.push(`- 基礎代謝: ${round(bmr)}kcal/日`)
  if (week.intakeDays > 0 && (week.activeAvg != null || bmr != null)) {
    const dailyBurn = (week.activeAvg ?? 0) + (bmr ?? 0)
    const dailyNet = (week.intakeAvg ?? 0) - dailyBurn
    const sign = dailyNet > 0 ? '+' : ''
    lines.push(`- 1日あたり差分: ${sign}${round(dailyNet)}kcal (${dailyNet < 0 ? '赤字=不足' : '黒字=超過'})`)
  }
  lines.push('')
  ...
  if (weight.start || weight.end) {
    lines.push('【体重 (この週)】')
    if (weight.start) {
      lines.push(`- 週初め頃 (${formatDateLabel(dayKey(new Date(weight.start.measured_at)))}): ${weight.start.weight_kg}kg`)
    }
    if (weight.end) {
      lines.push(`- 週終わり頃 (${formatDateLabel(dayKey(new Date(weight.end.measured_at)))}): ${weight.end.weight_kg}kg`)
    }
    if (weight.delta != null) {
      const sign = weight.delta > 0 ? '+' : ''
      lines.push(`- 1週間の変化: ${sign}${weight.delta.toFixed(1)}kg`)
    }
    lines.push('')
  }
...
export const inspectWeeklyAdvice = async ({ weekStart, modelId }) => {
  const [cached, context] = await Promise.all([
    getCachedWeeklyAdvice(weekStart),
    buildAdviceContextForWeek(weekStart),
  ])
  const hash = computeSnapshotHash({ context, modelId })
  return {
    cached,
...
export const generateWeeklyAdvice = async ({ weekStart, llm, modelId }) => {
  const context = await buildAdviceContextForWeek(weekStart)
  const hash = computeSnapshotHash({ context, modelId })
  const messages = buildAdviceMessages(context, 'weekly')
  const raw = await llm.generate(messages)
  const text = stripThink(raw)
  if (!text) throw new Error('空の応答が返りました')
  await saveWeeklyAdvice({ weekStart, snapshotHash: hash, adviceText: text, modelId })
  return { text, hash }
}
