...
| 食品データ (完成料理) | カロリーSlism (https://calorie.slism.jp) | public repo には含めない (`.gitignore` 配下)。約 6,690 件、ラーメン・寿司・パスタ料理・コンビニ商品等を補完 (`source='slism'`)。生 HTML/派生 JSON はローカルでビルド時にのみ存在 |
...
| 1.5. 完成料理・調理済み食品 | ペペロンチーノ、餃子、ラーメン、お好み焼き等 | カロリーSlism から同梱 (約 6,690 件、`source='slism'`)。public repo 配布外 (`.gitignore` 配下)。八訂の素材中心では弱い「家庭料理・外食・コンビニ商品」を補完。八訂と Slism に同名がある場合は八訂優先でマッチ |
