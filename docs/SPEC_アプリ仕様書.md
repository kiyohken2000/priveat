...
| 食品データ (素材中心) | 文部科学省 日本食品標準成分表（八訂）増補2023年 | 無料・公式・出典記載で利用可。約 2,500 件。Excel→JSON 変換して `assets/data/foods.json` で同梱 (`source='mext'`) |
| 食品データ (完成料理) | カロリーSlism (https://calorie.slism.jp) | **個人利用範囲のみ** (私的複製、著 30 条)。public repo には含めない (`.gitignore` 配下)。約 6,690 件、ラーメン・寿司・パスタ料理・コンビニ商品等を補完 (`source='slism'`)。生 HTML/派生 JSON はローカルでビルド時にのみ存在 |
...
| 層 | 対象 | 実現方法 |
| --- | --- | --- |
| 1. 素材・基本食品 | ごはん、鶏むね肉、バナナ等 | 文科省 日本食品標準成分表（八訂）増補2023年 を SQLite に同梱。約 2,500 食品。**出典表記必須**（後述）。`foods.source='mext'` |
| 1.5. 完成料理・調理済み食品 | ペペロンチーノ、餃子、ラーメン、お好み焼き等 | カロリーSlism から **個人利用範囲** で同梱 (約 6,690 件、`source='slism'`)。public repo 配布外 (`.gitignore` 配下)。八訂の素材中心では弱い「家庭料理・外食・コンビニ商品」を補完。八訂と Slism に同名がある場合は八訂優先でマッチ |
| 2. 市販パッケージ商品 | コンビニ商品等 | ラベルOCRで読取 → `products` にキャッシュして個人用DBを育てる。バーコードがあれば Open Food Facts を任意のオンライン補助に |
| 3. 外食・複合料理 (Slism 外) | チェーン店独自メニュー等 | `dishes` 概算テーブル（少数同梱）＋ ユーザーによる確認・カスタム登録。Slism にもない料理が中心 |
...
- 食品成分データ利用時は出典を明記すること:
  ...
  （アプリ内のクレジット／設定画面等に記載）
- **カロリーSlism データ** (`assets/data/foods_slism.json`) は個人利用範囲のみ。public repo (GitHub) には同梱しない (`.gitignore` 配下)。スクレイプ生 HTML (`scripts/data/slism_raw/`) も同様に gitignore 済み。第三者への再配布や商用展開時は、引用要件 (著 32 条) を超えるため Slism 運営に許諾を取り、出典「カロリーSlism (https://calorie.slism.jp) より引用」を明記する必要がある。
