...
llama.cpp 互換の GGUF VLM 候補:

| モデル | パラメータ | 量子化 | サイズ目安 | 想定 RAM | 日本語 | 備考 |
|---|---|---|---|---|---|---|
| **SmolVLM-Instruct 256M** | 256M | Q4_K_M | ~200MB | ~600MB | △ | 最軽量。料理認識精度未知 |
| **SmolVLM-Instruct 500M** | 500M | Q4_K_M | ~350MB | ~900MB | △ | 軽量・新しい (2025) |
| **Qwen2-VL-2B-Instruct** | 2B | Q4_K_M | ~1.5GB | ~2.5GB | ○ | Qwen 系で日本語強い、料理認識実績多い |
| **MiniCPM-V-2.6 8B** | 8B | Q4_K_M | ~5GB | ~7GB | ◎ | 認識精度トップクラスだが iPhone 一般機種ではキツい |
| **LLaVA-1.6 Mistral 7B** | 7B | Q4_K_M | ~4GB | ~6GB | △ | 旧モデル、日本語弱め |

### ✅ 決定 A: 主軸モデル — **両方サポート**

ModelScreen の「写真」タブで以下 2 つを選択式にする:
- **Qwen2-VL-2B-Instruct (Q4_K_M)** — 中庸。日本語料理認識で実用ラインに乗りやすい想定。高スペック端末 (RAM 6GB+) 推奨
- **SmolVLM-Instruct 500M (Q4_K_M)** — 軽量重視。低スペック端末でも動く。実機で精度ダメなら Qwen2-VL-2B にユーザーが切替

`getRecommendation` の vision ロールは tier に応じて (low: SmolVLM-500M, mid/high: Qwen2-VL-2B 推奨) のロジックを入れる。
