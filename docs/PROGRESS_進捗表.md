...
- **VLM で「料理名 + 推定 kcal」を 1 発で返す**（#130 後続）
  - 現状: parser 側は LLM kcal 推定 (`estimated_kcal`) を持ち、DB ヒットしない料理にも「(推定)」付きで kcal を出せる (#130)。VLM 経路はまだ DB マッチのみで、ヒットしない料理は「— kcal」のまま。
  - 案: Qwen3-VL-2B など出力長に余裕のある VLM に「料理名と推定 kcal を一発で返す」プロンプトを与え、`vlm` 経路でも `kcalSource='llm_estimate'` の流路を有効化。
  - 関連: `apps/mobile/src/scenes/chat/Chat.js` の `handlePhotoForVision` 内 enrich、`apps/mobile/src/data/llmModelsVlm.js`。
