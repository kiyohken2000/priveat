...
- **FoodCard の料理名インライン編集 + 行削除 + DB 同期**（#121 関連）
  - 現状: FoodCard は portion (small/normal/large) ピル切替しかできず、料理名が間違っていても直せない。実例: VLM が「ステーキ定食」を「ステーキ/ラーメン/味噌汁」と誤認識 (本来は「ステーキ/ごはん/味噌汁」)、テキストでも「キーマカレー → カレーライス」のような取り違えが起きる。
  - 設計: 料理名タップ → インライン TextInput → blur 確定。編集後 `findBestFood` で再マッチして `baseKcal` / `matchedName` / `matchedFoodId` を更新。あわせて行削除ボタン (誤認識を消すユースケース) と、`food_log` UPDATE API を追加 (現状 INSERT のみで portion 変更も DB 反映されていないので、料理名と portion の DB 同期をまとめて修正)。
  - スコープ外 (後回し): 数量・単位の編集 / モーダル UI 案。

- **DB ヒットしない料理の LLM kcal 推定**（#120 中期、Slism 統合の後段）
  - 現状: Slism 統合 (約 6,690 件) で家庭料理・外食・コンビニ商品の大半はカバーできる見込みだが、それでも Slism にもない料理 (チェーン店独自メニュー、地域料理、変則的な調理法) は残る。その場合 `baseKcal=null` で「— kcal」表示に戻ってしまう。
  - 本質的解決: parser LLM の出力 JSON に `estimated_kcal` フィールドを追加し、「DB に無さそうなら常識的な範囲で 1 つだけ整数で返す」と指示する。`food_log` に `kcal_source` カラム ('db' / 'llm_estimate' / 'manual') を追加し、FoodCard で LLM 推定値には「(推定)」バッジを必ず表示する (法的・健康的な誤認防止)。
  - 注意点: ハルシネーション対策として temperature 0.1 + 範囲指定プロンプト。VLM 経路では VLM 自身に「料理名 + 推定 kcal」を一発で返させる方が自然 (Qwen3-VL-2B なら可能)。
  - 関連: `docs/SPEC_アプリ仕様書.md` の `## 7. 食品DB戦略` に kcal 推定を最終フォールバック層として位置付ける案も検討。
...
- **AdviceCard の初回生成が「model not loaded」で失敗する**
  - 症状: ホーム/DayDetail で「AI からのアドバイスを生成」ボタンを押すと、初回だけ `WARN [adviceCard] generate failed: [Error: The model is currently not loaded. Please load the model before calling generate().]` で失敗する。2 回目以降は正常に生成される。
  - 推定原因: coach モデルのロード完了を待たずに `llm.generate` が呼ばれている (parser モデルがロード済みでも coach 役は別インスタンス、もしくは AdviceCard の `isReady` チェック漏れ)。
  - 暫定回避: ユーザーがもう一度ボタンを押せば成功する。
  - 修正方針: `AdviceCard` 側で `llm.isReady` (もしくは coach role の ready フラグ) を確認してから generate を呼ぶ。未 ready ならロード完了を待つかボタンを disable する。
