...
八訂は素材中心 (約 2,500 件) で完成料理 (餃子・ラーメン・寿司・パスタ料理・コンビニ商品) のカバー率が低いため、カロリーSlism (約 6,690 件) を同梱する。public repo には含めない (`.gitignore` 配下)。
...
- [x] 公開前: App Store / Google Play での "Priveat" 名称重複・商標の確認 (2026-06-08 調査)
  - **ストア検索**: App Store JP/US, Google Play JP の **完全一致は 0 件**。
  - **要注意**: **PRIVEAT TECHNOLOGIES, Inc.** (US 法人 / [Crunchbase](https://www.crunchbase.com/organization/priveat-technologies) / [LinkedIn](https://www.linkedin.com/company/priveat) / [priveat.netlify.app](https://priveat.netlify.app/)) が **食領域 (シェフ / レストラン at-home dining marketplace) で実在**。 US は使用主義 (common law trademark) なので、 登録が無くても実使用で保護が発生しグローバル配信は紛争リスクあり。
  - **判断**: 改名はせず、 **ストア配信地域を日本のみに絞る** 運用で当面回避。 日本では商標登録 / アプリ / Web 検索のいずれにも痕跡なし (リスク低)。
  - **将来**: グローバル展開を検討する段階で再評価 → 必要なら改名 (まだ未リリースなのでこの時点の改名コストは大きくない)。
- [x] ランディングページ + プライバシーポリシー / 利用規約の作成・公開 (2026-06-08)
  - `apps/web/` に Vite + React + TypeScript + React Router でランディング 3 ページ (`/` / `/privacy` / `/terms`) を実装。 React Router 7 + プレーン CSS + CSS Modules、 Noto Sans JP、 mobile の brand purple (#8100ff / #231d54) で配色統一。
  - スマホスクショ 25 枚 (機能 9 セクション分) を WebP 540px 幅にリサイズして `apps/web/public/screenshots/` に配置。 個別の機能 (記録モード / 今日のサマリー / コーチ / 履歴 / レシピ / マイ食品 / 設定 / LLM モデル / ヘルス連携) ごとに紹介。
  - プライバシーポリシーは「全データ端末内・第三者提供なし・お問い合わせは retwpay@gmail.com」を中心に簡潔化。 利用規約は個人開発・無償・自己責任・日本法準拠ベース。
  - **Cloudflare Pages** にデプロイ完了 (project: `priveat`): https://priveat.pages.dev/
  - SPA fallback (`public/_redirects`) 設定済みで `/privacy` `/terms` への直接アクセスも 200。
- [ ] （公開する場合）ストア申請準備：Apple Developer / Google Play 登録
  - **配信地域: 日本のみ** で申請する (上記名称調査の結論に基づく)。 App Store Connect / Play Console の Availability 設定で対応。
  - **プライバシーポリシー URL**: https://priveat.pages.dev/privacy （ストアコンソールに登録）
...
- **追加検討: Gemma 系 GGUF をベンチマークで比較し低スペック parser 候補を採用 (#167)** ✅ 完了 (2026-06-07)
  ...
  追記 (2026-06-09): Google 公式の QAT Q4_0 GGUF コレクション (`google/gemma-4-qat-q4-0`) を確認。
  モバイル候補は `google/gemma-4-E2B-it-qat-q4_0-gguf` (~3GB) と `google/gemma-4-E4B-it-qat-q4_0-gguf` (~4.5GB) の 2 つ。
  ただし Google org は HF gated のままなので、 `expo-file-system` 直接 DL は不可。 再評価するなら以下のいずれか:
    1. bartowski / unsloth が Q4_0 公式版を再配布するのを待つ (前回 E2B は unsloth UD-Q4_K_XL を使った)
    2. `llmTextModelStorage.js` の DL に HF トークン Authorization ヘッダを付ける対応を入れる
  なお前回見送りの主因は thinking mode の reasoning 占有なので、 公式 Q4_0 にしても `enable_thinking: false` 前提でないと結論は変わらない見込み。
  E4B は 4.5GB で `TIER_4GB` では現実的でなく、 評価対象に入れるなら `TIER_6GB`/`TIER_8GB` gating が必要。
  参考 collection: `transformers` / `ct` (compressed-tensors) フォーマットの `gemma-4-qat-mobile` 系は llama.rn では使えない (vLLM / MediaPipe / LiteRT 向け)。

- **マイ食品マスタを設定から CRUD 可能にし、 サジェスト元に統合 (#180)** ✅ 実装完了 (2026-06-08)

  動機: ラベル OCR 経由で `products` テーブルに登録された食品 (`insertProductFromLabel`, `source='label_ocr'`) は再利用先が `LabelRecordCard` の一回限りの記録のみで、 一覧 / 編集 / 削除する画面が無かった。 また「ラベルを持っていない (パッケージ忘れ・コンビニ品の手書き登録)」ケース向けに手入力経路も無かった。 既存 products テーブルをそのままマイ食品マスタとして CRUD UI を被せ、 FoodNameInput のサジェスト元にもマージして記録モード + RecipeEditScreen の両方から再利用できるようにする。

  ### 実装サマリ

  - [x] **db/products.js 新設** (`listProducts` / `getProduct` / `insertProductManual` / `updateProduct` / `searchProductsByName` / `findProductByExactName`)。 `deleteProductRow` は既存 `ocrLogs.js` (画像 image_uri を一緒に消す) を re-export。
  - [x] **`findBestFood` / `computeKcalFromMatch` を products 対応に拡張**。 優先順: recipe 完全一致 → **product 完全一致** → alias → food あいまい。 `adaptProductAsMatch` で `kind='product'` + `kcal_per_serving=product.kcal` に正規化し、 `computeKcalFromMatch` の serving 系単位フォールバックに乗せる。 g 単位指定は products に per_100g が無いので非対応 (= null)。
  - [x] **FoodNameInput のサジェスト統合**。 `searchFoodsByName` と `searchProductsByName` を並列実行 → products を先頭にマージ → `kind='product'` の行には「マイ食品」 / 「ラベル」 バッジ (source で出し分け)。 候補タップ時に products は `serving_desc` を suggestedUnit に使う (無ければ「個」)。
  - [x] **`ProductsScreen.js` 新設** (設定 → マイ食品)。 `listProducts` で取得、 image_uri があればサムネ表示、 `kcal / serving_desc / source / created_at` をメタ行。 「＋ 新しく登録」 ボタンで ProductEditScreen を新規モードで開く。 空状態あり。 `useFocusEffect` で再読込。
  - [x] **`ProductEditScreen.js` 新設**。 `route.params.id` の有無で新規/編集を分岐。 name / serving_desc / kcal / P / F / C / salt / barcode を編集、 ラベル画像があればヘッダにサムネ表示、 削除ボタン (Alert 確認 → `deleteProductRow`)。 kcal 必須・他は任意。
  - [x] **SettingsHome に「マイ食品」 行追加** (cube アイコン)。 既存「自炊レシピ」 と 「LLM モデル」 の間に配置。 SettingsStacks に `ProductsScreen` / `ProductEditScreen` を登録。
  - [x] **新規登録時にラベル画像アップロード + OCR 自動入力**。 ProductEditScreen (新規モードのみ) に「ラベルを撮影」 / 「写真から選ぶ」 ボタンを設置。 既存 `imageOcr.js` (`captureFromCamera` / `pickFromLibrary` / `runOcr`) + `ocrParsers.js` の `parseLabelText` を再利用し、 抽出できた kcal/P/F/C/salt を state にマージ。 OCR 失敗/抽出ゼロでも画像プレビューと手入力は残る。 `insertProductManual` を `(fields, { imageUri, source })` 形に拡張し、 画像付き登録は `source='label_ocr'` に統一 (チャット経路の `insertProductFromLabel` と整合)。 編集モードでの差し替えは未対応 (別タスク)。
  - [x] **単位 AI 推定**。 「1 単位の表示」 欄の隣に「AI 推定」 ボタンを追加。 `aiKcal.js` に `estimateUnitForFood` を新設 (候補リスト: `個 / 袋 / 本 / 枚 / 缶 / 杯 / 切れ / 玉 / パック / g / mL`)。 LFM2.5-JP は `<unit>` タグや CoT を使わず短い自然文で返す傾向が強いので、 user prompt を `{name} → ` の completion 形式にし、 system prompt に商品カテゴリ別ヒント + じゃがりこ/ポテチ等の固有名詞 few-shot を入れて raw が単語 1 語で返るよう調整 (実機で「じゃがりこL → 袋」「ポテチ → 袋」「プレーンヨーグルト → パック」 を確認)。 RecipeEditScreen の AI 推定と同じ 2 段階モデルスワップ動線を流用。 プレースホルダーも「例: 1個 / 1袋 / 100g」 → 「例: 個 / 袋 / 本 / g / グラム」 に変更。
  - [x] **1 単位の運用ヒントを UI に追加**。 ヨーグルト 400g パックを 200g 食べるようなケース向けに「商品 1 パックではなく、 自分が普段 1 回で食べる量を 1 単位にする」 と補助テキストで明示。 products テーブルが per_serving 想定 (per_100g カラムなし) なのを、 ユーザーの常用量に合わせる運用で吸収する設計。 厳密な per_100g 対応 (スキーマ v9 + computeKcal 拡張) は別タスク。

  ### 関連ファイル

  - `apps/mobile/src/db/products.js` (新規)
  - `apps/mobile/src/db/search.js` (`findBestFood` 拡張 + `adaptProductAsMatch` export + `computeKcalFromMatch` の product 分岐)
  - `apps/mobile/src/components/FoodNameInput.js` (products マージ + バッジ表示)
  - `apps/mobile/src/scenes/settings/ProductsScreen.js` (新規)
  - `apps/mobile/src/scenes/settings/ProductEditScreen.js` (新規)
  - `apps/mobile/src/scenes/settings/SettingsHome.js` (行追加)
  - `apps/mobile/src/routes/navigation/stacks/SettingsStacks.js` (ルート 2 つ追加)

  ### 残課題 / 別タスク

  - **チャットからの手入力経路 (新モード追加 β)**: 「セブンのサラダチキン 113kcal P25 F1.5 C1」 のような自由文を parser に判定させて products に登録するフロー。 設定 → マイ食品 → 新規追加で代替可能なので必要性を見てから判断。
  - **編集モードでの画像差し替え**: 新規モードは OCR 経路あり、 編集モードは既存 image_uri をプレビューするだけ。 撮り直したいなら現状は「削除して新規登録」 で代替。 必要になれば後日。
  - **products と food の重複対策 (例: 牛乳)**: 現状サジェストには両方並ぶ。 ユーザーが選んで使い分けられるので問題視されるまで放置。

- **マイ食品 / 自炊レシピ一覧に名前フィルタ追加 (#173 / #180 後続)** ✅ 実装完了 (2026-06-08)

  動機: マイ食品・レシピが増えてきたときに探しにくくなる前に、 一覧画面の上部でインクリメンタルに絞り込みできる UI を入れておきたい (memo 経由の依頼)。 list は 200 件上限で取得済みなので DB 再クエリは不要、 `useMemo` でクライアントサイド絞り込み。

  ### 実装サマリ

  - 共通: 上部に「名前で絞り込み」 TextInput。 `normalize = (s) => String(s ?? '').replace(/[\s　]/g, '').toLowerCase()` で全半角スペース除去 + 小文字化して `includes` 判定 (全角スペースはリテラルだと eslint `no-irregular-whitespace` を踏むので `　` エスケープ)。 `ScrollView` に `keyboardShouldPersistTaps="handled"` を付けて IME 中の chip タップを潰さない。
  - `ProductsScreen.js` のみ source チップ `[すべて / 手入力 / ラベル OCR]` を併設。 source ラベルは行メタ行で既に表示済みなので軸として自然。 active 時は `colors.lightPurple` 背景 + 白文字、 inactive は白背景 + gray 文字 + 薄ボーダ。
  - `RecipesScreen.js` は source 軸が無いので名前フィルタのみ。
  - 空状態を 2 種に分離: 「登録 0 件」 (登録方法を案内、 cube/cutlery アイコン) と 「条件に一致しない」 (search アイコン + 短い文言)。 ProductsScreen は同一画面内で `&&` 並列描画 (no-nested-ternary 回避)、 RecipesScreen は 0 件は full-page centered のまま。

  ### 関連ファイル

  - `apps/mobile/src/scenes/settings/ProductsScreen.js` (+117/-32)
  - `apps/mobile/src/scenes/settings/RecipesScreen.js` (+96/-32)

- **記録モード parser 精度退化の修復 + few-shot を engine 別に分岐 + kind=unknown 救済** ✅ 実装完了 (2026-06-08)

  動機: 「カツ丼と缶チューハイ2本」 のような多品目入力が `bb2b451` 以降に 3 モデル (qwen3-0.6b / LFM2.5-1.2B-JP / Gemma3-1B) 全てで欠落 / 構造崩壊するようになっていた (Phase 3 DoD では完璧に動いていた)。 原因は qwen3 のハルシネーション対策で多品目 few-shot を撤去した副作用 — 1 品目だけの例に絞ったため、 「と」 連結の複数品目を学習させる例が消えていた。

  ### 修復方針

  - `FEW_SHOT` を `FOOD_SINGLE` / `FOOD_MULTI` / `TAIL` に分割し `buildFewShot(engine)` で組立。
  - executorch (qwen3-0.6b) は単品例のみ (多品目例を入れると 「Failed to generate text」 や 1 token で停止する `2f63dbd` の症状が再現するため)。 入れ子バグは `schema.js` の `flattenWrappedItems` で 1 品目だけ救済を継続。
  - llama.rn (LFM2.5 / Gemma3) はカツ丼と缶チューハイ2本の 2 品目例を 1 件追加。 これで両モデルが「と」 連結を正しく分解できるようになる (実機ベンチで確認)。
  - `buildSystemPrompt(engine)` シグネチャ変更、 呼び出し側 (Chat.js 2 箇所 + BenchmarkScreen.js) は `activeEngine` を渡す。
  - schema.js: `kind=unknown` で items が埋まっているケース (LFM2.5 が「分類自信なしで kind=unknown を選びつつ items を埋める」 挙動) を food として救済。

  ### 副次的に変更

  - coach デフォルトモデルを `qwen3-1.7b-q` → `qwen3-0.6b-q` に統一 (parser と揃える)。 既存ユーザーは AsyncStorage の値が優先なので影響なし。

  ### 関連ファイル

  - `apps/mobile/src/scenes/chat/Chat.js` (few-shot 分割 / buildSystemPrompt(engine))
  - `apps/mobile/src/scenes/chat/schema.js` (kind=unknown 救済)
  - `apps/mobile/src/scenes/settings/BenchmarkScreen.js` (engine ごとに systemPrompt 組み立て)
  - `apps/mobile/src/state/modelContext.js` (coach デフォルト統一)

  コミット: `33a3d24` + `18df4cb`

- **記録モード parser を 2-stage 化 (BenchmarkScreen POC で検証、 本実装は別タスク)** 🧪 POC 完了 (2026-06-08)

  動機: 上記 修復後も 4372 chars の単発プロンプトでは小型モデル (特に qwen3-0.6b) が think に時間を取られて 11-18s かかる + 多品目入力でたまに 1 品目に潰す。 「kind 分類 → kind 別の詳細抽出」 の 2 段に分けると各ステージが短い prompt で動くため、 トータル時間が短縮 + 各 stage の few-shot を絞れて精度が上がる仮説。

  ### 設計

  - **Stage 1**: ルールベース分類器 (正規表現、 LLM 不使用、 ~1ms)
    - recipe: 「食分」 OR 「人前」 + 「作」 系動詞
    - activity: 運動動詞 (走/歩/泳/筋トレ/サイクリング 等)
    - weight: 「体重」 キーワード OR 純数値 (体重相場 30-200kg、 任意 kg 単位)
    - その他: food (最頻出かつ多様なので fallback)
    - LLM 版 stage 1 も実装したが、 「鶏むね200g」 を qwen3-0.6b → weight、 LFM2.5 → recipe と 2/3 モデルが誤分類して破綻 (description に literal で書いてあっても効かない)。 食事アプリの入力ドメインは constrained でキーワードが強いので、 正規表現の方が高速 + 正確だった。
  - **Stage 2**: kind 別のフォーカスされたプロンプトで LLM を 1 回叩く
    - `STAGE2_FOOD_PROMPT` (~700 chars): `{"items":[...]}` だけ出力。 kind フィールドを含めないことで qwen3 の入れ子バグ (`{"kind":"food","items":[{"kind":"food","items":[...]}]}` のような自己再帰) を構造的に回避。
    - `STAGE2_WEIGHT_PROMPT` (~400 chars): `{"weight_kg":<数値>}`
    - `STAGE2_ACTIVITY_PROMPT` (~400 chars): `{"activity_name":..,"duration_min":..,"distance_km":..}`
    - recipe は既存 `buildRecipeSystemPrompt()` を再利用
    - unknown は LLM 不要、 即座に return
  - **parseStage2Output**: stage2 出力に kind を補完してから `parseRecordOutput` に流す (kind=food は legacy path で kind 不要、 他は kind を JSON head に injection)

  ### 実機測定結果 (2026-06-08、 iPhone)

  単発 (修復後) vs 2-stage (rule+LLM)、 単位は秒。 「× n品目」 は parse OK の品目数。

  | 入力 | qwen3-0.6b (single → 2-stage) | LFM2.5 (single → 2-stage) | Gemma3 (single → 2-stage) |
  |---|---|---|---|
  | 鶏むね200g (food 1 品) | 15.4s ×1 → **3.1s ×1** | 7.1s ×1 → 2.0s ×1 | 6.8s ×1 → 2.6s ×1 |
  | 体重68.5kg (weight) | 11.3s OK → **1.3s OK** | 7.0s OK → 0.8s OK | 7.1s OK → 0.9s OK |
  | 30分で3キロ走った (activity) | 16.7s OK → **2.2s OK** | 7.3s OK → 1.5s OK | 8.4s OK → 1.6s OK |
  | カツ丼と缶チューハイ2本 (food 2 品) | 11.6s ×1 (融合) → 4.9s ×2 | 7.9s ×2 → 2.7s ×2 | 9.4s ×1 → 2.5s ×2 |
  | ごはん大盛りとバナナ1本と焼き魚 (food 3 品) | 18.5s ×1 → 3.4-7.2s ×1〜3 (ばらつき) | 9.7s NG (unknown) → 3.3s ×3 | 10.5s ×1 → 3.9-4.1s ×3 |

  全 15 パターン (5 入力 × 3 モデル) で 2-stage が速度・精度ともに同等以上 (qwen3 ごはん は変動あるが、 食事アプリの主要入力で 3-9x 高速化)。 UX 影響大: 鶏むね200g 入力時の体感が **15 秒 → 3 秒**。

  ### POC スコープ

  - BenchmarkScreen にトグル UI 追加 (parser モードを「単発 / 2-stage」 で切替)
  - `Chat.js` に `classifyByRules` / `STAGE2_FOOD_PROMPT` / `STAGE2_WEIGHT_PROMPT` / `STAGE2_ACTIVITY_PROMPT` を追加 (export 済み、 本実装で Chat.js parser 経路から呼び出す予定)
  - **本実装 (Chat.js mode='log' / 'recipe' の 2-stage 化) は別タスク**。 chatConfig.systemPrompt + history の経路を脱却して `llm.generate()` 直叩きへ切替が必要 (parser は本来 single-turn 抽出なので history 不要)

  ### 関連ファイル

  - `apps/mobile/src/scenes/chat/Chat.js` (classifyByRules + stage2 プロンプト 3 種、 export)
  - `apps/mobile/src/scenes/settings/BenchmarkScreen.js` (2-stage 実行ヘルパ + UI トグル + 結果カード拡張)

  ### 残課題

  - qwen3-0.6b の カツ丼 quantity bleed (融合 1 品目 → quantity ズレ 2 品目への改善はしたが、 単位 / 数量の不一致は残る)。 UI で編集可能なので許容
  - llama.rn (LFM2.5/Gemma3) のカツ丼 unit エラー (杯 → 本)。 STAGE2_FOOD から 「カツ丼と缶チューハイ2本」 few-shot を抜いたため。 qwen3 退化リスクの方が大きかったので採用しない判断

- **記録モード parser 2-stage 本実装 (Chat.js 統合)** ✅ 完了 (2026-06-08)

  動機: 上記 POC で実証した速度/精度改善 (3-9x 高速化) を Chat 画面の本番経路にも反映する。 POC は BenchmarkScreen のスタンドアロン実行だけだったので、 実ユーザーが触る Chat 画面では従来の単発プロンプト経路のままだった。

  ### 設計判断: 4b (llm.sendMessage を介さず llm.generate 直叩き)

  選択肢を 2 つ検討:
  - **4a (最小変更)**: parser モードでも `llm.sendMessage` を使い続け、 送信前に `configure({systemPrompt: stage2, initialMessageHistory: []})` で都度上書き → parseAndDispatch に kind を渡す。 変更箇所 3 ヶ所で済むが、 各送信ごとに `llm.messageHistory` が空になり **チャットスクロールの過去 parser メッセージが消える** (UX 退化)。
  - **4b (推奨・採用)**: `llm.sendMessage` を経由せず `llm.generate([{system, user}])` を直叩き。 user/assistant の IMessage は全て `localMessages` に push して GiftedChat が描画。 parser は本来 single-turn 抽出なので history を持つ必要が無いという 構造的事実に正直な設計。 変更は広範だが、 履歴汚染が原理的に起きない + チャット履歴 UX を完全維持。

  → 4b を採用。 parser を chat history 経路から切り離すことで、 過去の 「rev 増し付け回避」「configure 競合」 「mode 切替時の history 復元」 系の複雑さからも段階的に解放される。

  ### 主要な変更

  - `apps/mobile/src/scenes/chat/schema.js` に `parseStage2Output(rawOutput, kind)` を移動 (POC で BenchmarkScreen ローカルだったものを export)
  - `apps/mobile/src/scenes/chat/Chat.js`:
    - `parseAndDispatch` を `parseRecordOutput` + 新規 `enrichParsedRecord` に分離。 enrichParsedRecord は kind 別 DB enrichment + mode 整合性チェック (recipe ⇄ food 降格 等) を担う。 `idPrefix` 引数で foodItems[].id を可変化 (history index `idx` でも localMessages stamp でも動く)。
    - `runParserTwoStage(text, mode)`: ルール分類 → stage2 プロンプト → `llm.generate()` → parseStage2Output → enrichParsedRecord。 mode='recipe' は kind を 'recipe' で固定。
    - `handleParserResult(result, mode)`: 結果 kind に応じて `synthFoodCardMessage` / `synthRecipeCardMessage` / `synthWeightCardMessage` / `synthActivityCardMessage` / `synthParserErrorMessage` を localMessages に push。 food のみ即時 `insertFoodLogItems` で DB 書き込み (foodLogId を貼る)。
    - `onSend` を mode で分岐: `coach` は従来通り `llm.sendMessage`、 `log` / `recipe` は `localMessages` に user message を積んで `runParserTwoStage` を呼ぶ。
    - 新規 synth ヘルパ: `synthFoodCardMessage` / `synthRecipeCardMessage` / `synthWeightCardMessage` / `synthActivityCardMessage` / `synthParserErrorMessage` (IMessage に foodItems / recipe / weightRecord / activityRecord フィールドを直貼り)。
    - FoodCard 編集ハンドラ群 (`findFoodItemSnapshot` / `applyFoodItemPatch` / `removeFoodItemRow` / `handleEstimateMissingKcal`) の prefix チェックを `local-card-` / `local-vlm-` 限定から `local-` に広げ、 新しい `local-food-` / `local-recipe-` などにも対応。
    - RecipeCard 編集ハンドラ群 (`applyRecipePatch` / `updateRecipeIngredientQuantity` / `deleteRecipeIngredient` / `handleSaveRecipe` / `handleEstimateMissingRecipeKcal`) を `h-` 限定から localMessages 両対応に拡張 (新規 `findRecipeSnapshot` / `applyRecipeIngredientsPatch` で共通化)。
    - `handleWeightSave` / `handleActivitySave` の saved 状態反映を `h-` 限定から `local-` 両対応に拡張。
    - `getStage2PromptFor(kind)` を export (BenchmarkScreen と共有)。
  - `apps/mobile/src/scenes/settings/BenchmarkScreen.js`: `parseStage2Output` を schema.js から import (POC ローカル定義を削除)、 `getStage2Prompt` も `getStage2PromptFor` 経由に整理。

  ### 動作

  - 記録モード / レシピモードで送信 → `local-user-<stamp>` の IMessage が即座にチャットに追加 → `llm.isGenerating=true` で typing indicator → stage2 完了で結果カードが追加。
  - 既存のチャット履歴 (mode 切替前の localMessages snapshot) はそのまま保持される (handleSetMode の localMessages snapshot 機構を変更せず流用)。
  - coach モードは無変更で動作。

  ### 旧経路の縮退

  - 旧 `parseAndDispatch` は coach 専用 useEffect (line ~1090) からの呼び出しのみ残存 (parser 経路では呼ばれない)。 ただし parser モードで llm.messageHistory への assistant 行 push は発生しないため、 防御的に loop を残してあるが空回りする。
  - `chatConfig.systemPrompt` の parser システムプロンプト configure は依然として走るが、 sendMessage を呼ばないので副作用なし。 将来的に parser 経路は configure 自体スキップ可能。

  ### 動作確認で発見した hallucination バグ + 修正

  実機テストで 「お腹すいた」 を送信したところ、 LFM2.5-1.2B-JP が STAGE2_FOOD_PROMPT の few-shot 例 「ごはん大盛りとバナナ1本と焼き魚」 を 丸ごとコピーして出力 → 誤データが food_log に INSERT される問題が発生。
  - 根本原因: `enrichParsedRecord` の food 分岐の hallucination filter が、 **全件 hallucination されたケース**で `parsed.items` (元の全件) にフォールバックしてしまっていた。 「無水カレー一食」 で parser が 「カレー」 「ごはん」 を返すような部分 hallucination の救済策が、 完全 hallucination で裏目に出る形。
  - 修正: `userText && filteredItems.length === 0` なら `{ kind: 'unknown' }` に倒す。 ユーザーには 「食事・体重・運動のいずれにも判定できませんでした」 エラーカードが出るので、 誤った food_log INSERT が防げる。
  - 部分 hallucination (「無水カレー一食」 → 「カレー」 だけ残る) は filteredItems.length ≥ 1 なので従来通り動作。
  - 旧経路 (`parseAndDispatch` 呼び出しで userText='') は `isItemNameInUserInput` が常に true を返すため filteredItems = parsed.items となり、 ここには来ない (旧経路の挙動は維持)。

  ### 関連ファイル

  - `apps/mobile/src/scenes/chat/Chat.js`
  - `apps/mobile/src/scenes/chat/schema.js`
  - `apps/mobile/src/scenes/settings/BenchmarkScreen.js`

### 既知の不具合（後回しでよい）

- **TextInput の IME 未確定文字の下線が出ない**（iOS / Android 両方、全 TextInput が対象）→ **上流 RN Fabric リグレッション、修正待ち** (2026-06-08 調査)
  ...
  - **原因確定**: RN core の Fabric (新アーキ) リグレッション。 RN 0.76.3 〜 0.80+ の複数バージョンで再現報告あり (うちの 0.85.3 も該当)。
    - 参照: [facebook/react-native#55257](https://github.com/facebook/react-native/issues/55257) (Japanese Market Blocker), [expo/expo#38422](https://github.com/expo/expo/issues/38422), [callstack/react-native-paper#4779](https://github.com/callstack/react-native-paper/issues/4779)
    - 修正候補 PR: [facebook/react-native#56082](https://github.com/facebook/react-native/pull/56082) (open / 未マージ、 ターゲット版未定)
  - **唯一の確認済み回避策**: `app.json` の `newArchEnabled: false`。 ただしうちは **reanimated 4.x が新アーキ必須** なので、 旧アーキへ戻すには reanimated 3.x ダウングレード + その API 差異吸収が要る → 下線 1 個のために巻き込むのは割に合わず採用しない。
  - **今後の方針**: 上流 PR #56082 のマージ待ち。 RN/Expo SDK アップグレード時に同 PR が取り込まれていないか確認する。 patch-package で先取り適用は PR 確定後に検討。
