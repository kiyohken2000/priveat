...
- **DB ヒットしない料理の LLM kcal 推定**（#120 関連）
  - 現状: `findBestFood` で DB マッチに失敗すると `baseKcal=null` で「— kcal」表示になる (例: 「餃子」は成分表に「ぎょうざ」表記でしか登録されていないため漢字入力で外れる)。ユーザー視点では「分からない」より「概算で出してくれる」方が圧倒的に有用。
  - 短期 (即効): `data/foodAliases.js` に頻出料理 50 件程度 (餃子, ラーメン, カツ丼, ハンバーグ, 寿司, 焼き魚, パスタ, ピザ…) を追加して成分表コードに紐付け。コミット 1 回で大幅改善できる。
  - 中期 (本質的解決): parser LLM の出力 JSON に `estimated_kcal` フィールドを追加し、「DB に無さそうなら常識的な範囲で 1 つだけ整数で返す」と指示する。`food_log` に `kcal_source` カラム ('db' / 'llm_estimate' / 'manual') を追加し、FoodCard で LLM 推定値には「(推定)」バッジを必ず表示する (法的・健康的な誤認防止)。
  - 注意点: ハルシネーション対策として temperature 0.1 + 範囲指定プロンプト。VLM 経路では VLM 自身に「料理名 + 推定 kcal」を一発で返させる方が自然 (Qwen3-VL-2B なら可能)。
  - 関連: `docs/SPEC_アプリ仕様書.md` の `## 7. 食品DB戦略（3層）` に kcal 推定を 4 層目として位置付ける案も検討。
